...
namespace AuthorizationData.Main.Repositories
{
  public class UserInfoRepository : IDisposable
  {
    ...
    public async Task<bool> AddUserInfoAsync(UserInfoDb user)
    {
      ...
      return await _contextMain.Database.ExecuteSqlCommandAsync( $"addUser {user.UserId}, {user.Login}, {user.BirthDate}, {user.Sex}, {user.City},  {user.Description}, {user.FullAvatarLink}, {user.SmallAvatarLink}, {returnsCode} OUT") > 0;
    }

    public async Task<bool> UpdateUserInfoAsync(UserInfoDb user)
    {
      return await _contextMain.Database.ExecuteSqlCommandAsync( $"updateUserTableData {user.UserId}, {user.Login}, {user.BirthDate}, {user.Sex}, {user.City},  {user.Description}, {user.FullAvatarLink}, {user.SmallAvatarLink}") > 0;
    }
